...
jd() {
  ...
  __jd_sanitize() { echo "$1" | sed -E 's/[ ]+/_/g; s#/+#_#g'; }
  # Parse a TSV line into variables: typ id disp path parent
  __jd_parse_line() {
    local line="$1"
    # Use zsh's read with IFS to avoid external tools
    local IFS=$'\t'
    read -r typ id disp path parent <<< "$line"
    printf '%s\t%s\t%s\t%s\t%s\n' "$typ" "$id" "$disp" "$path" "$parent"
  }
  __jd_extract_code_from_display() { echo "$1" | sed -E 's/^\s*[▸▾] ?([0-9][0-9]([-][0-9][0-9]|\.[0-9][0-9])?)\b.*/\1/; t; s/.*/ /'; }
  __jd_is_code_item() { [[ "$1" =~ ^[0-9]{2}\.[0-9]{2}$ ]]; }
  __jd_is_code_cat() { [[ "$1" =~ ^[0-9]{2}$ ]]; }
  __jd_is_code_range() { [[ "$1" =~ ^[0-9]{2}-[0-9]{2}$ ]]; }

  __jd_find_category_id_by_code() {
    local code="$1"
    local line
    while IFS= read -r line; do
      local parsed; parsed=$(__jd_parse_line "$line") || continue
      local IFS=$'\t'; read -r typ id disp path parent <<< "$parsed"
      if [[ "$typ" == dir && "$disp" == *" $code "* || "$disp" == $code\ * || "$disp" == *\ $code ]]; then
        echo "$id"; return 0
      fi
    done < <(jd-helper tree "${ROOTS[@]}" --state "$STATE")
    return 1
  }

  if [[ -z "$1" ]]; then
    # Build a literal reload command without eval; assumes paths have no spaces
    local RELOAD_CMD_LIT="jd-helper tree ${ROOTS[*]} --state $STATE"
    local sel
    ...
    sel=$(jd-helper tree "${ROOTS[@]}" --state "$STATE" | \
      env FZF_DEFAULT_OPTS="$FZF_OPTS" fzf --with-nth=3 --delimiter="\t" \
        ...
        --bind "tab:execute-silent(jd-helper toggle --state \"$STATE\" --id {2})+reload($RELOAD_CMD_LIT)" \
        --bind "enter:accept" \
        --bind "change:reload(
   if [ -n \"{q}\" ]; then
     exec ${TREE_ALL}
     ...
     exec ${TREE_BASE}
        ...
        --bind "ctrl-a:execute-silent(jd-helper expand-all --state \"$STATE\" ${ROOTS[*]})+reload($RELOAD_CMD_LIT)" \
        --bind "ctrl-g:execute-silent(jd-helper reset-state --state \"$STATE\")+reload($RELOAD_CMD_LIT)" \
        --bind "alt-n,ctrl-n:execute(jd-helper new-interactive --parent-id {2} --display \"{3}\" ${ROOTS[@]} </dev/tty >/dev/tty 2>&1)+reload($RELOAD_CMD_LIT)" 
