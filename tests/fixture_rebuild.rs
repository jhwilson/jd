...
fn recreate_t99_into(dest: &Path) {
    ...
    let status = Command::new(&script_path).arg(dest).status().unwrap();
...
fn rebuild_fixture_and_scan() {
    ...
    assert!(dest
        .join("99-99_Test_Range/99_TestCat/99.01_TestItem")
        .exists());
    assert!(dest
        .join("99-99_Test_Range/99_TestCat/99.02_Example.url")
        .exists());
    assert!(dest
        .join("99-99_Test_Range/99_TestCat/99.03_Website.webloc")
        .exists());
