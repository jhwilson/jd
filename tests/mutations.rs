...
fn cargo_bin() -> Command {
    Command::cargo_bin("jd-helper").unwrap()
}

fn set_home(cmd: &mut Command, home: &PathBuf) {
    cmd.env("HOME", home);
}
...
fn mutate_dir_file_link_rename_move_delete() {
    ...
    let root = td.path().join("R50_Research");
    ...
    fn find_by_suffix(node: &serde_json::Value, path_end: &str) -> Option<String> {
        ...
        if let Some(arr) = node.get("children").and_then(|c| c.as_array()) {
            for ch in arr {
                if let Some(x) = find_by_suffix(ch, path_end) {
                    return Some(x);
                }
            }
        }
    ...
    if let Some(arr) = v.get("roots").and_then(|r| r.as_array()) {
        for r in arr {
            if let Some(x) = find_by_suffix(r, "30-39_Research/30_Topic") {
                cat_id_opt = Some(x);
                break;
            }
        }
    ...
    let file_id = {
        ...
        fn find(node: &serde_json::Value) -> Option<String> {
            if node
                .get("path")
                .and_then(|s| s.as_str())
                .map(|s| s.ends_with("30.02_Note.txt"))
                .unwrap_or(false)
            {
                return node
                    .get("id")
    ...
    };
    {
        ...
        cmd.assert().success();
        assert!(root
            .join("30-39_Research/30_Topic/30.02_Renamed.txt")
            .exists());
    }
    ...
    let item_dir_id = {
        ...
        fn find(node: &serde_json::Value, path_end: &str) -> Option<String> {
            ...
            if let Some(arr) = node.get("children").and_then(|c| c.as_array()) {
                for ch in arr {
                    if let Some(x) = find(ch, path_end) {
                        return Some(x);
                    }
                }
            }
        ...
        if let Some(arr) = v.get("roots").and_then(|r| r.as_array()) {
            for r in arr {
                if let Some(x) = find(r, "30.01_ItemDir") {
                    got = Some(x);
                    break;
                }
            }
    ...
    };
    {
        ...
        cmd.assert().success();
        assert!(root
            .join("30-39_Research/30_Topic/30.01_ItemDir/30.02_Renamed.txt")
            .exists());
    }
    ...
    let link_id = {
        ...
        fn find(node: &serde_json::Value) -> Option<String> {
            if node
                .get("path")
                .and_then(|s| s.as_str())
                .map(|s| s.ends_with("30.03_Link.url"))
                .unwrap_or(false)
            {
                return node
                    .get("id")
...
fn move_under_category_keeps_underscores_in_title() {
    ...
    fs::write(
        root.join("30-39_Research/30_Topic/30.01_Two_Word_Note.txt"),
        b"x",
    )
    .unwrap();
    ...
    fn find_id(node: &serde_json::Value, path_end: &str) -> Option<String> {
        ...
        if let Some(arr) = node.get("children").and_then(|c| c.as_array()) {
            for ch in arr {
                if let Some(x) = find_id(ch, path_end) {
                    return Some(x);
                }
            }
        }
    ...
    let file_id = roots
        .iter()
        .find_map(|r| find_id(r, "30.01_Two_Word_Note.txt"))
        .unwrap();
    let cat_id = roots
        .iter()
        .find_map(|r| find_id(r, "30-39_Research/31_Other"))
        .unwrap();
    ...
    assert!(root
        .join("30-39_Research/31_Other/31.01_Two_Word_Note.txt")
        .exists());
}
